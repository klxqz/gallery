...
class photosGalleryPlugin extends photosPlugin {

    public function saveSettings($settings = array()) {
        if (empty($settings['albums'])) {
            $settings['albums'] = array();
        }
        $settings['albums'] = json_encode($settings['albums']);
        
        if (empty($settings['subalbums'])) {
            $settings['subalbums'] = array();
        }
        $settings['subalbums'] = json_encode($settings['subalbums']);
    ...
    public function frontendPhoto($photo) {
        ...
        $sidebar .= '<p><span class="p-rate" data-vote-url="' . $frontend_vote_url . '">' .
                ...
                '</span></p>';
        $sidebar .= '<script>$(function() { $.photos.galleryInitRateWidget(); });</script>';
    ...
    public function backendAssets() {
        ...
        return "<style>#sidebar-gallery-plugin-awaiting { margin-top: 10px; } </style>";
    }
    ...
    public function preparePhotosBackend(&$photos) {
        if (wa()->getUser()->getRights('photos', 'edit')) {
            foreach ($photos as &$p) {
                if ($p['source'] == 'gallery') {
                    // get author
                    $contact = new waContact($p['contact_id']);
                    $author = array(
                        'id' => $contact['id'],
                        'name' => photosPhoto::escape($contact['name']),
                        'backend_url' => wa()->getConfig()->getBackendUrl(true) . 'contacts/#/contact/' . $contact['id']
                    );

                    $links = array(
                        'Загружено: <a href="' . $author['backend_url'] . '">' . $author['name'] . '</a><br/>
    ...
    public function backendPhoto($photo_id) {
        ...
        if ($photo) {
            ...
            $html = '<a class="hint" href="javascript:void(0);" id="photo-rate-votes-count" data-you-voted="' . (int) ($your_rate > 0) . '"><u>' . $votes_count_text . '</u></a>' .
                    ...
                    '</a></span>' .
                    '<a class="inline-link p-rate-clear small" href="javascript:void(0);" style="display:none;" id="clear-photo-rate"><b><i>' . _wp('cancel my vote') . '</b></i></a>';
            $html .= '<script>$.photos.galleryInitYourRate();</script>';
            return array(
                'after_rate' => $html
            );
        }
    ...
    public function prepareCollection($params) {
        if (isset($params['hash']) && $params['hash'][0] == 'gallery' && $params['hash'][1] == 'myphotos') {
            $id = wa()->getUser()->getId();
            if ($id) {
                /**
    ...
    public function extraPrepareCollection($params) {
        if (isset($params['hash'][1]) && strstr($params['hash'][1], 'moderation=') !== false) {
            ...
            if (isset($search[1])) {
                ...
                $params['collection']->addWhere("source='gallery'");
                return;
            }
        }
